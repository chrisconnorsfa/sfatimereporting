...
/**
 * Polyfill for forEach which is only supported by IE9+.
 * 
 * http://jsperf.com/native-array-foreach-vs-polyfill
 */
if (!Array.prototype.forEach) {
    Array.prototype.forEach = function(func, ctx) {
        var i = 0
          , self = Object(this)
          , length = self.length
...
function ResponsiveDatatablesHelper(tableSelector, breakpoints) {
    ...
    this.expandIconTemplate = '<a class="responsiveExpander"></a>';
...
ResponsiveDatatablesHelper.prototype.createExpandIcon = function (tr) {
    ...
    if (this.expandColumn !== undefined && this.expandColumn < tds.length) {
        ...
        if ($('a.responsiveExpander', td).length == 0) {
            td.prepend(this.expandIconTemplate);

            // Respond to click event on expander icon.
            td.on('click', 'a.responsiveExpander', {responsiveDatatablesHelperInstance: this}, this.showRowDetailEventHandler);
        }
    }
};
...
ResponsiveDatatablesHelper.prototype.disable = function (disable) {
    ...
    if (this.disabled) {
        ...
        $('tbody tr a.responsiveExpander', this.tableElement).remove();
        ...
        this.tableElement.off('click', 'a.responsiveExpander', this.showRowDetailEventHandler);
    }
}
